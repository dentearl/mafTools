...
static bool debug = false;  // FIXME: tmp, make a flag
...
static struct malnComp *createJoinedRefComp(struct malnBlk *blkJoined, struct malnComp *refComp1, struct malnComp *refComp2, struct malnCompCompMap *srcDestCompMap) {
    ...
    malnCompCompMap_insert(srcDestCompMap, refComp1, comp);
    malnCompCompMap_insert(srcDestCompMap, refComp2, comp);
    return comp;
}

/* add non-reference components to the joined blk and destination component
 * array */
static void addCompsToJoined(struct malnBlk *blkJoined, struct malnBlkCursor *blkCursor, struct malnComp **destComps, struct malnCompCompMap *srcDestCompMap) {
    ...
    for (int i = 1; i < blkCursor->numRows; i++) {
        ...
        malnCompCompMap_insert(srcDestCompMap, blkCursor->rows[i].comp, destComps[i]);
    }
    // FIXME: simpler but slower to only use the hash instead of the arrays?
}
...
struct malnBlk *malnJoinBlks(struct malnComp *refComp1, struct malnComp *refComp2, struct malnComp **joinedCompRet) {
    ...
    if (debug) { // FIXME: tmp
        malnComp_dump(refComp1, "refComp1", stderr);
        malnBlk_dump(blk1, "inBlk1", stderr);
        malnComp_dump(refComp2, "refComp2", stderr);
    ...
    // blkCursors, with the first row being the join row
    ...
    struct malnBlk *blkJoined = malnBlk_construct();
    destComps1[0] = destComps2[0] = createJoinedRefComp(blkJoined, refComp1, refComp2, srcDestCompMap);
    addCompsToJoined(blkJoined, blkCursor1, destComps1, srcDestCompMap);
    addCompsToJoined(blkJoined, blkCursor2, destComps2, srcDestCompMap);
    ...
    if (debug) {malnBlk_dump(blkJoined, "joined@1", stderr);}
    copyUnsharedRefColumns(blkJoined, destComps2, blkCursor2, 0, aln2CommonStart);
    if (debug) {malnBlk_dump(blkJoined, "joined@2", stderr);}
    ...
    if (debug) {malnBlk_dump(blkJoined, "joined@3", stderr);}
    ...
    malnBlk_setTree(blkJoined, joinTrees(blkCursor1, blkCursor2, srcDestCompMap));
